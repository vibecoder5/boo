...
Локальная офлайн-читалка (Go 1.24 + встроенный `web/`). UI и ошибки — на русском. Нет своих аккаунтов и облака boo. Google Диск — по желанию, пока пользователь сам не подключит.
...
- `internal/store` — `%AppData%/boo/state.json` (+ `library/`, `covers/`, `dictionaries/`, `ui/`, `drive/`).
- `internal/drive` — OAuth и двусторонний синк файлов с Google Диском.
...
- Не слушать не-loopback. В сеть за данными пользователя — только Google Диск после явного подключения; токен в `drive/`, не в ZIP и не на Диск.
- Форматы сходятся в `epub.Book`; HTML глав — через `sanitizeChapter`.
- `/res` отдаёт только image/* и font/*.
- UI общий (`store.Data.UI`, словари); полка, прогресс, закладки, заметки, todos, history, статистика чтения, undo, списки — в текущем `Workspace`.
